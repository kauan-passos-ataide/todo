...
const Modal = ({
  ...
  const handleAddTask = useCallback(() => {
    const newSubTodo = {
      id:
        subTodoItem.length > 0
          ? Math.max(...subTodoItem.map((item) => item.id)) + 1
          : 1,
      label: titleToAdd,
      checked: false,
    };
        ...
        <div className="flex flex-col gap-3 select-none">
          <div className="flex flex-row gap-5 select-none">
            ...
            <button
              ...
              className="px-14 py-2 bg-gradient-zinc bg-[length:200%_200%] animate-gradient-move text-white rounded flex justify-center items-center hover:bg-zinc-900 select-none"
            >
              Salvar
            </button>
          </div>
          ...
          <div className="">
            {subTodoItem.map((item) => (
              <div key={item.id}>
                ...
                <div className="w-full select-none"></div>
              </div>
            ))}
          </div>
        </div>
