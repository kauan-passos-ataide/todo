...
const Home = () => {
  ...
  const handleChange = (
    title: string,
    date: Date | undefined,
    subTodoForAdd: SubTodo[],
    idToChange?: number,
    deleteItem?: boolean
  ) => {
    if (deleteItem) {
      setTodoItem((prevTodoItem) =>
        prevTodoItem.filter((item) => item.id !== idToChange)
      );
      return;
    }
    if (idToChange) {
        ...
        prevTodoItem.map((item, index) =>
          index + 1 === idToChange
            ? {
                ...item,
                label: title,
                createdAt: date ? date.toLocaleDateString("pt-BR") : date,
                checked: false,
                subTodo: subTodoForAdd,
              }
            : item
        )
      );
  ...
  const handleAddTask = useCallback(() => {
    const newTodo = {
      id:
        todoItem.length > 0
          ? Math.max(...todoItem.map((item) => item.id)) + 1
          : 1,
      label: titleToAdd,
      checked: false,
  ...
  const toggleItem = (itemIndex: number) => {
    setTodoItem(
      todoItem.map((item) =>
        item.id === itemIndex ? { ...item, checked: !item.checked } : item
      )
    );
  };
    ...
    setTodoItem(
      todoItem.map((item) =>
        item.id === itemIndex
          ? {
              ...item,
              subTodo: item.subTodo.map((subItem) =>
                subItem.id === subTodoIndex
        ...
        <div className="bg-white rounded-xl p-2 w-full min-h-32 -mt-32">
          ...
          <div>
            {filteredTodos.length > 0 ? (
                ...
                <div
                  ...
                  className={`${
                    expandedTask === item.id
                      ? "bg-gradient-white bg-[length:200%_200%] animate-gradient-move p-[2px] rounded-xl"
                      : "p-0"
                  }`}
                >
                  <div className="py-2 bg-white rounded-t-xl px-5 lg:px-14 flex justify-between border-b border-zinc-200 cursor-pointer">
                    ...
                    <div
                      className="w-full flex justify-start items-start px-5 whitespace-normal"
                      onClick={() => handleTaskClick(item.id)}
                    >
                      <span>{item.label}</span>
                    </div>
                    <div
                      className="w-12 border-l border-zinc-300 flex justify-end items-end"
                        ...
                        {item.subTodo.map((todo) => (
                          ...
                          >
                            <div className="py-2 px-4 lg:px-14 flex justify-between border-b-2 border-zinc-200 rounded-b-xl cursor-pointer">
                              ...
                              <div
                                className="w-12 border-l border-zinc-300 flex justify-end items-end"
                                onClick={() => toggleSubTodo(item.id, todo.id)}
                              >
              ...
              <p className="text-center text-zinc-500 pt-3">
              ...
              filteredNoChecked.map((item) => (
                ...
                >
                  <div className="py-2 px-5 lg:px-14 flex rounded-t-xl bg-white justify-between border-b-2 border-zinc-200 cursor-pointer">
                    ...
                    <div
                      className="w-full flex justify-start items-start px-5 text-zinc-700 line-through whitespace-normal"
                      onClick={() => handleTaskClick(item.id)}
                    >
                      <span>{item.label}</span>
                    </div>
